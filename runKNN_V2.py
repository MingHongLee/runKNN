# python -m streamlit run runKNN_V2.py
...
if 'playlist' not in st.session_state:
    st.session_state.playlist = pd.DataFrame(columns=['Song', 'Artist', 'Music Genre Tags'])

# If the user has entered a song name, perform the recommendation
if song_input:
    indices = recommender(song_input, X, knn10)
    ...
    if st.button('Add to Playlist'):
        if not selected_songs.empty or song_input:
            # Add the input song to the playlist
            if song_input:
                # Find the index of the input song
                input_song_idx = process.extractOne(song_input, df['name'])[2]
                input_song_data = {
                    'Song': df['name'][input_song_idx],
                    'Artist': df['artist'][input_song_idx],
                    'Music Genre Tags': df['tags'][input_song_idx]
                }
                input_song_df = pd.DataFrame([input_song_data])
                st.session_state.playlist = pd.concat([st.session_state.playlist, input_song_df], ignore_index=True)

            # Add the selected songs to the playlist
            st.session_state.playlist = pd.concat([st.session_state.playlist, selected_songs], ignore_index=True)
